-- # SELECT within SELECT tutorial - In this tutorial we will use SELECT statements within SELECT statements to perform more complex queries on the table world 
...
-- Then find the names of the countries associated with these continent
...
FROM world x
WHERE x.population >ALL(SELECT population*3 FROM world y WHERE y.continent = x.continent AND x.name<>y.name)
